...
2. Add the **exact same two lines** inside `<head>...</head>`:
...
- [ ] Step 4 (password page): Same two render lines also added to `<head>` in `layout/password.liquid` — it is a separate standalone layout and does **not** inherit from `theme.liquid`
